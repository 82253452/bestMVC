<template>
  <div class="app-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column align="center" label="编码" width="150">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="时间" width="150">
        <template slot-scope="scope">
          <span>{{getConfigTime(scope.row.typecode)}}</span>
        </template>
      </el-table-column>
...
<script>
  import {selectByPage, insert, selectById, updateById, deleteById} from '@/api/base/tcType'
  import {selectByPage as selectConfigAll} from '@/api/base/TConfig'
  ...
  export default {
    ...
    methods: {
      ...
      getConfigAll() {
        selectConfigAll(this.listQuery).then(response => {
          this.configAll = response.data.list
        })
      },
      getConfigTime(c) {
        for (const v of this.configAll) {
          if (v.subcode === c) {
            return v.subtime
          }
        }
      ...
      handleUpdate(row) {
        ...
        this.temp.time = this.getConfigTime(row.typecode)
